...
public final class DocumentFactoryImpl implements DocumentFactory {
	...
	@Override
	public Document mergeSlideDocument(Document template, Document content) {
	...
	private void mergeElementAttributes(Element destination, Element source) {
		source.attributes().entrySet().forEach(e -> destination.setAttribute(e.getKey(), e.getValue()));
	}

	private void mergeBindingNodes(Element destinationElement, Element sourceSubElement) {
	...
	private void mergeElements(Element destination, Element source) {
	...
	public Document createEventDataDocument() {
